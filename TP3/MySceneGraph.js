...
class MySceneGraph {
	...
	onXMLReady() {
		...
		const graph = this
		graph.scene.gameOrchestrator.themeManager.setActiveCamera()
		setTimeout(_ => {
			graph.loadedOk = true

			if (!graph.scene.gameOrchestrator.themeManager.applied) graph.scene.gameOrchestrator.themeManager.applyTheme()

			graph.scene.gameOrchestrator.prolog.response = 0
			graph.scene.gameOrchestrator.state = MyGameOrchestrator.getState().menu
			graph.scene.gameOrchestrator.camera.goToMenu()
			// this.scene.gameOrchestrator.camera.setPlayerCamera(MyGameOrchestrator.firstPlayer())
			graph.scene.setPickEnabled(true)
		}, 3000)
	...
	parseXMLFile(rootElement) {
		...
		// <board>
		if ((index = nodeNames.indexOf('board')) == -1) return this.onXMLError('tag <board> missing')
		else {
			if (index != BOARD_INDEX) this.onXMLMinorError('tag <board> out of order')

			//Parse textures block
			if ((error = this.parseBoard(nodes[index])) !== null) return error
		}
	...
	/**
	 * Parses the board.
	 * @param {Object} boardNode - the board node.
	 */
	parseBoard(boardNode) {
		const children = boardNode.children

		const boardElements = 7
		if (children.length != boardElements) return this.onXMLError('Board must have ' + boardElements + ' arguments')

		if (children[0].nodeName != 'boardNode')
			return this.onXMLError('The first tag of the board must be <boardNode>')

		if (children[1].nodeName != 'position') return this.onXMLError('The second tag of the board must be <position>')

		if (children[2].nodeName != 'whiteDice')
			return this.onXMLError('The third tag of the board must be <whiteDice>')

		if (children[3].nodeName != 'blackDice')
			return this.onXMLError('The fourth tag of the board must be <blackDice>')

		if (children[4].nodeName != 'smallCaveNode')
			return this.onXMLError('The fifth tag of the board must be <smallCaveNode>')

		if (children[5].nodeName != 'largeCaveNode')
			return this.onXMLError('The sixth tag of the board must be <largeCaveNode>')

		if (children[6].nodeName != 'mountain') return this.onXMLError('The sixth tag of the board must be <mountain>')

		this.database.boardNodeID = this.reader.getString(children[0], 'id')
		const position = this.parseCoordinates4D(children[1], 'board position')
		const whiteDicePath = this.reader.getString(children[2], 'path')
		const blackDicePath = this.reader.getString(children[3], 'path')
		this.database.smallCaveNodeID = this.reader.getString(children[4], 'id')
		this.database.largeCaveNodeID = this.reader.getString(children[5], 'id')
		this.database.mountainNodeID = this.reader.getString(children[6], 'id')

		this.scene.gameOrchestrator.gameboard = new Board(this.scene, whiteDicePath, blackDicePath, position)
