...
class NodeParser {
	...
	parseNodes(nodesNode) {
		...
		for (var i = 0; i < children.length; i++) {
			...
			// verifies if it is the board
			if (nodeID == this.parser.database.boardNodeID) {
				this.parser.database.boardParsed = true
				this.parser.database.boardNode = node
			}
			// verifies if it is the small cave
			if (nodeID == this.parser.database.smallCaveNodeID) {
				this.parser.database.smallCaveParsed = true
				this.parser.scene.gameOrchestrator.gameboard.createSmallCaves(node)
			}
			// verifies if it is the large cave
			if (nodeID == this.parser.database.largeCaveNodeID) {
				this.parser.database.largeCaveParsed = true
				this.parser.scene.gameOrchestrator.gameboard.createLargeCave(node)
			}
			// verifies if it is the mountain
			if (nodeID == this.parser.database.mountainNodeID) {
				this.parser.database.mountainParsed = true
				this.parser.scene.gameOrchestrator.gameboard.createMountains(node)
			}
		}
		...
		// verifies if the board node is defined
		if (!this.parser.database.boardParsed)
			return this.parser.onXMLError('board node ' + this.parser.scene.database.boardNodeID + ' not defined')

		// verifies if the small cave node is defined
		if (!this.parser.database.smallCaveParsed)
			return this.parser.onXMLError('small cave node ' + this.parser.database.smallCaveNodeID + ' not defined')

		// verifies if the small cave node is defined
		if (!this.parser.database.largeCaveParsed)
			return this.parser.onXMLError('large cave node ' + this.parser.database.largeCaveNodeID + ' not defined')

		// verifies if the mountain node is defined
		if (!this.parser.database.mountainParsed)
			return this.parser.onXMLError('mountain node ' + this.parser.database.mountainNodeID + ' not defined')
	...
	parseLeaf(node, textureWrapper, nodeID) {
		...
		if (primitive == 'objfile') return this.parseObjFile(node, nodeID)
	 ...
	 * @param {Object} node		- the node to parse.
	 * @param {string} nodeID	- the node ID.
	 * @returns {*} a obj file on success, otherwise on error.
	 */
	parseObjFile(node, nodeID) {
		let path = this.parser.reader.getString(node, 'path')
		if (path == null) return this.parser.onXMLError('no obj file path defined in node ' + nodeID)

		let wireframe = this.parser.reader.getInteger(node, 'wireframe')
		wireframe = wireframe ? true : false

		return new CGFOBJModel(this.parser.scene, path, wireframe)
	}
}
